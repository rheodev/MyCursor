        ...
        <Card.Content>
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <label className="text-sm font-medium whitespace-nowrap" style={{ color: "var(--text-secondary)" }}>服务端口:</label>
              <Input type="number" value={port.toString()} onChange={(e) => { const v = parseInt(e.target.value, 10); if (!isNaN(v) && v > 0 && v < 65536) setPort(v); }} className="w-32" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Button variant="primary" onClick={handleInject} loading={actionLoading === "inject"} disabled={!!actionLoading} className="h-16 flex-col" icon={<Icon name="bolt" size={20} />}>
                {status?.injected ? "重新注入" : "注入无感换号"}
              </Button>
              <Button variant="danger" onClick={handleRestore} loading={actionLoading === "restore"} disabled={!!actionLoading || !status?.backup_exists} className="h-16 flex-col" icon={<Icon name="refresh" size={20} />}>
                恢复原始文件
              </Button>
            </div>
          </div>
      ...
      <Card>
        ...
        <Card.Content>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Button variant="info" onClick={handleStartServer} loading={actionLoading === "start"} disabled={!!actionLoading || (status?.server_running ?? false)} className="h-14" icon={<Icon name="login" size={20} />}>
                启动服务器
              </Button>
              <Button variant="danger" onClick={handleStopServer} loading={actionLoading === "stop"} disabled={!!actionLoading || !(status?.server_running ?? false)} className="h-14" icon={<Icon name="logout" size={20} />}>
                停止服务器
              </Button>
            </div>
          </div>
              ...
              {lastResult.details && lastResult.details.length > 0 && (
                <ul className="space-y-1">
                  {lastResult.details.map((d, i) => (
                    <li key={i} className="text-xs font-mono" style={{ color: "var(--text-secondary)" }}>{d}</li>
                  ))}
                </ul>
              )}
            </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <span className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: "var(--primary-color)", color: "white" }}>
                  {i + 1}
                </span>
...
function StatusBadge({ label, active, activeText, inactiveText }: { label: string; active: boolean; activeText: string; inactiveText: string }) {
  return (
    <div className="p-3 rounded-lg" style={{ backgroundColor: "var(--bg-secondary)" }}>
      <p className="text-xs font-medium mb-1" style={{ color: "var(--text-tertiary)" }}>{label}</p>
      <div className="flex items-center gap-2">
        <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: active ? "#4ec9b0" : "#666" }} />
