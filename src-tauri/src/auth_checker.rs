...
impl AuthChecker {
    ...
    /// 构造 Cursor Dashboard API 通用请求头（含完整浏览器指纹）
    fn build_cursor_dashboard_headers() -> Result<reqwest::header::HeaderMap> {
        let mut h = reqwest::header::HeaderMap::new();
        h.insert("Accept", "application/json, text/plain, */*".parse()?);
        h.insert("Accept-Encoding", "gzip, deflate, br, zstd".parse()?);
        h.insert("Accept-Language", "en,zh-CN;q=0.9,zh;q=0.8,eu;q=0.7".parse()?);
        h.insert("Content-Type", "application/json".parse()?);
        h.insert("Origin", "https://cursor.com".parse()?);
        h.insert("Referer", "https://cursor.com/dashboard".parse()?);
        h.insert("Sec-CH-UA", "\"Not;A=Brand\";v=\"99\", \"Google Chrome\";v=\"139\", \"Chromium\";v=\"139\"".parse()?);
        h.insert("Sec-CH-UA-Arch", "\"x86\"".parse()?);
        h.insert("Sec-CH-UA-Bitness", "\"64\"".parse()?);
        h.insert("Sec-CH-UA-Mobile", "?0".parse()?);
        h.insert("Sec-CH-UA-Platform", "\"macOS\"".parse()?);
        h.insert("Sec-CH-UA-Platform-Version", "\"15.3.1\"".parse()?);
    ...
    /// 构造 WorkosCursorSessionToken Cookie 值
    ///
    /// 优先使用账户中保存的 workos_cursor_session_token；
    /// 若无，则使用占位 userId 拼接 accessToken（与 CursorPool_Client 方案一致）。
    fn build_workos_cookie(access_token: &str) -> String {
        // 优先从已保存的账户记录中查找完整的 WorkOS Session Token
        if let Ok(accounts) = crate::account_manager::AccountManager::load_accounts() {
            for account in &accounts {
                if account.token == access_token {
                    if let Some(ref wt) = account.workos_cursor_session_token {
                        if !wt.is_empty() {
                            log_info!("🔑 使用账户中保存的 WorkOS Session Token");
        ...
        // Fallback: 用占位 userId + accessToken 拼接
        // 服务端只校验 :: 后面的 token 部分，userId 可以是任意合法格式
        ...
        log_info!("🔑 使用 userId 拼接 accessToken 构造 Cookie");
        format!(
            "WorkosCursorSessionToken=user_01000000000000000000000000%3A%3A{}",
            token_part
        )
    }
    ...
    async fn get_account_info(
        ...
        subscription_headers.insert("Referer", "https://cursor.com/dashboard".parse()?);
        subscription_headers.insert(
            "Sec-CH-UA",
                    ...
                    match resp.text().await {
                        Ok(body) => {
                            ...
                            if let Ok(json_data) = serde_json::from_str::<serde_json::Value>(&body)
                            {
                                ...
                                // 优先使用 individualMembershipType（真实会员类型），没有则用 membershipType
                                let sub_type = json_data.get("individualMembershipType")
                                    .and_then(|v| v.as_str())
                                    .filter(|s| !s.is_empty())
                                    .or_else(|| json_data.get("membershipType").and_then(|v| v.as_str()));
                                if let Some(t) = sub_type {
                                    account_info.subscription_type = Some(t.to_string());
                                    log_info!("📋 订阅类型: {}", t);
                                }
        ...
        let mut usage_headers = Self::build_cursor_dashboard_headers()?;
        usage_headers.insert("Cookie", Self::build_workos_cookie(token).parse()?);
        ...
        // 获取聚合用量数据（近30天）
        let cookie = Self::build_workos_cookie(token);
        let end_date = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_millis() as u64;
        let start_date = end_date - (30 * 24 * 60 * 60 * 1000);

        match Self::get_aggregated_usage_data(&cookie, start_date, end_date, -1, details).await {
    ...
    pub async fn get_usage_for_period(
        ...
        let cookie = Self::build_workos_cookie(token);
        Self::get_aggregated_usage_data(&cookie, start_date, end_date, team_id, &mut details).await
    }
    ...
    pub async fn get_user_analytics(
        ...
        let mut details = Vec::new();
        let cookie = Self::build_workos_cookie(token);
        Self::get_user_analytics_data(&cookie, team_id, user_id, start_date, end_date, &mut details).await
    }
    ...
    pub async fn get_usage_events(
        ...
        let mut details = Vec::new();
        let cookie = Self::build_workos_cookie(token);
        Self::get_filtered_usage_events(&cookie, team_id, start_date, end_date, page, page_size, &mut details).await
    }
